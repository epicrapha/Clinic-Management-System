        ...
        <div class="row align-items-center">
            <div class="col-lg-1 col-4">
                <a href="#!" class="header-logo">
                    <img src="{{ asset(getAppLogo()) }}" alt="Infy Care" class="object-cover front-app-logo"
                        loading="lazy" />
                </a>
            </div>
                            ...
                            @if(getLogInUser())
                                @if(getLogInUser()->hasRole('doctor'))
                                    <a href="{{ route('doctors.dashboard') }}"
                                        class="btn btn-outline-primary me-xxl-3 me-2 mb-3 mb-lg-0"
                                ...
                                @endif
                            @else
                                <a href="{{ route('login') }}" class="btn btn-outline-primary me-xxl-3 me-2 mb-3 mb-lg-0"
                                    data-turbo="false">{{ __('messages.login') }}</a>
                            @endif

                            <a href="{{ route('medicalAppointment') }}"
                                class="btn btn-primary mb-3 mb-lg-0">{{ __('messages.web.book_an_appointment') }}</a>
                        </div>
                    </div>
                </nav>
            </div>
