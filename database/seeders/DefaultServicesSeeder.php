...
class DefaultServicesSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        foreach ($input as $data) {
            $image = $data['icon'];
            unset($data['icon']);
            $service = Service::updateOrCreate(['name' => $data['name']], $data);
            $service->serviceDoctors()->sync($doctors);

            if (!$service->hasMedia(Service::ICON)) {
                if (str_starts_with($image, 'http')) {
                    ...
                    $service->addMediaFromPath(public_path($image))->toMediaCollection(Service::ICON, config('app.media_disc'));
                }
            }
        }
    }
